...
{
    queue<int> q;
    for (int i = 1; i <= n; i++) d[i] = inf, dd[i] = 0;
    d[S] = 0;
    dd[S] = 1;
    q.push(S);
    while (!q.empty())
    ...
    for (int i : g[u])
    {
        ...
        if (cl[v] != id && d[v] == d[u] + l)
        {
            if (E[i].capa > E[i].flow)
            {
                ...
                {
                    E[i].flow += delta;
                    E[4 * m+1 - i].flow -= delta;
                    if(i<=m)
                    {
                        E[i+m].flow-=delta;
                        E[4*m+1-(i+m)].flow+=delta;
                    }
                    else
                    {
                        E[i-m].flow-=delta;
...
L1:
            ...
            if (i == 1)
                for (int j = 1; j <= sl; ++j) printf("%d ", path[j]);
            if (i == 2)
                for (int j = sl - 1; j >= 1; j--) printf("%d ", path[j]);
        }
    }
}
